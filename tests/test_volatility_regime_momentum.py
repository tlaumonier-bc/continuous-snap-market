"""Tests for the volatility-regime momentum model."""
...
def test_model_is_registered_and_builds():
    ...
    model = build_model("volatility_regime_momentum", features, shared_parameters(),
                        fast_volatility_regime_momentum_parameters())
    assert model.name == "volatility_regime_momentum"
...
def test_evaluation_starts_after_the_first_full_window():
    features = synthetic_features()
    parameters = fast_volatility_regime_momentum_parameters()
    model = build_model("volatility_regime_momentum", features, shared_parameters(), parameters)
...
def test_volatility_conditioning_changes_the_displayed_curve():
    ...
    one_regime = build_model(
        "volatility_regime_momentum", features, shared,
        parameters.__class__(**{**parameters.__dict__, "volatility_bin_count": 1}))
    two_regimes = build_model("volatility_regime_momentum", features, shared, parameters)
